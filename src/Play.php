    ...
    public static function checkLivingNeighbors($cellNeighbors){
        ...
        foreach($cellNeighbors as $line){
            ...
            foreach($line as $cell){
                if(isset($cellNeighbors[$i-1][$j-1]) && $cellNeighbors[$i-1][$j-1]->isAlive === true){
                    $cell->livingNeighbors++;
                }
                if(isset($cellNeighbors[$i-1][$j]) && $cellNeighbors[$i-1][$j]->isAlive === true){
                    $cell->livingNeighbors++;
                }
                if(isset($cellNeighbors[$i-1][$j+1]) && $cellNeighbors[$i-1][$j+1]->isAlive === true){
                    $cell->livingNeighbors++;
                }
                if(isset($cellNeighbors[$i][$j-1]) && $cellNeighbors[$i][$j-1]->isAlive === true){
                    $cell->livingNeighbors++;
                }
                if(isset($cellNeighbors[$i][$j+1]) && $cellNeighbors[$i][$j+1]->isAlive === true){
                    $cell->livingNeighbors++;
